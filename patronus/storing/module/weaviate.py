...
class WeaviateDocStore(BaseDocStore):
    ...
    2. Allows combination of vector search and scalar filtering,
    i.e. you can filter for a certain tag and do dense retrieval on that subset
    3. Has less variety of ANN algorithms, as of now only HNSW.
    4. Requires document ids to be in uuid-format.
    If wrongly formatted ids are provided at indexing time they will be replaced with uuids automatically.
    ...
    def __init__(
        ...
        :param host: Weaviate server connection URL for storing and processing documents and vectors.
                             For more details, \
                                refer "https://weaviate.io/developers/weaviate/current/getting-started/installation.html"
        ...
        :param index: Index name for document text, embedding and metadata (in Weaviate terminology, \
            this is a "Class" in Weaviate schema).
        ...
        :param similarity: The similarity function used to compare document vectors. \
            Available options are 'cosine' (default), 'dot_product' and 'l2'.
        ...
        if similarity == "cosine":
            ...
            raise ValueError(
                f"It looks like you provided value '{similarity}' for similarity in the WeaviateDocumentStore constructor. \
                Choose one of these values: 'cosine', 'l2', and 'dot_product'"
            )
        ...
        if self.custom_schema:
            ...
            schema = {
                "classes": [
                    {
                        "class": index,
                        "description": "Index, it's a class in Weaviate",
    ...
    def _convert_weaviate_result_to_document(self, result: dict, return_embedding: bool, scale_score: bool = True) -> Document:
        """
    ...
    def write(
        ...
        :param documents: List of `Dicts` or List of `Documents`. \
            A dummy embedding vector for each document is automatically generated if it is not provided. \
            The document id needs to be in uuid format. \
            Otherwise a correctly formatted uuid will be automatically generated based on the provided id.
    ...
    def search(
        ...
        :param scale_score: Whether to scale the similarity score to the unit interval (range of [0,1]).
                            If true (default) similarity scores (e.g. cosine or dot_product) which naturally
                            have a different value range will be scaled to a range of [0,1], where 1 means extremely relevant.
    ...
    def query_by_embedding(
        ...
        :param return_embedding: To return document embedding
        :param scale_score: Whether to scale the similarity score to the unit interval (range of [0,1]).
                            If true (default) similarity scores (e.g. cosine or dot_product)
                                which naturally have a different value range will be scaled to a range of [0,1],
                                    where 1 means extremely relevant.
                            Otherwise raw similarity scores (e.g. cosine or dot_product) will be used.
        :return:
        """
        if headers:
        ...
        documents = []
        for result in results:
            doc = self._convert_weaviate_result_to_document(result, return_embedding=return_embedding, scale_score=scale_score)
            documents.append(doc)
    ...
    def delete_index(self, index: str):
        ...
        if index == self.index:
            logger.warning(
                ...
                f"If you plan to use this index again, please reinstantiate '{self.__class__.__name__}' \
