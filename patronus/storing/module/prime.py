...
from patronus.etc.schema import Answer, Document, Label
...
class MetaLabelORM(ORMBase):
    ...
    __table_args__ = (
        ForeignKeyConstraint([label_id, label_index], [LabelORM.id, LabelORM.index], ondelete="CASCADE", onupdate="CASCADE"),
        {},
    )  # type: ignore


class SQLDocStore(BaseDocStore):
    def __init__(
        self,
        url: str = "sqlite://",
        index: str = "document",
    ...
    def _get_documents_meta(self, documents_map):
        ...
        meta_query = self.session.query(MetaDocumentORM.document_id, MetaDocumentORM.name, MetaDocumentORM.value).filter(
    ...
    def delete_index(self, index: str):
        ...
        :return: None
        """
        SQLDocStore.delete_documents(self, index)
